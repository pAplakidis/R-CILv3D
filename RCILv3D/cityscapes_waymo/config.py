...
def add_all_arguments(parser, train):
    ...
    parser.add_argument('--use_ranger', action='store_true', default=False, help='whether to use Ranger variant of ADAM')
    parser.add_argument('--use_gelu', action='store_true', default=False, help='whether to replace all ReLU activation functions with GELU')
    ...
    if train:
        ...
        parser.add_argument('--exclude_test_3dnoise', action='store_true', default=False, help='Exclude 3D noise from generator during test phase')
    return parser
...
def set_dataset_default_lm(opt, parser):
    if opt.dataset_mode == 'ade20k':
    ...
    elif opt.dataset_mode == 'cityscapes-waymo':
        parser.set_defaults(lr_g=0.0002)
        ...
        print(f'Warning: {opt.dataset_mode} has no defined config.')
