...
class DP_GAN_Generator(nn.Module):
    def __init__(self, opt):
        ...
        if not self.opt.no_3dnoise:
            ...
            self.seg_pyrmid.append(
                nn.Sequential(
                    nn.Conv2d(self.opt.semantic_nc + self.opt.z_dim, 32, 3, stride=1, padding=1),
                    nn.BatchNorm2d(32),
                    nn.ReLU(inplace=True) if not opt.use_gelu else nn.GELU()
                )
            )
        else:
            ...
            self.seg_pyrmid.append(
                nn.Sequential(
                    nn.Conv2d(self.opt.semantic_nc, 32, 3, stride=1, padding=1),
                    nn.BatchNorm2d(64),
                    nn.ReLU(inplace=True) if not opt.use_gelu else nn.GELU()
                )
            )
    ...
    def forward(self, input, z=None):
        ...
        if not self.opt.no_3dnoise:
            ...
            if self.opt.phase == 'test' and self.opt.exclude_test_3dnoise:
                z = torch.zeros(seg.size(0), self.opt.z_dim, dtype=torch.float32, device=dev)
            else:
...
class ResnetBlock_with_SPADE(nn.Module):
    def __init__(self, fin, fout, opt):
        ...
        self.activ = nn.LeakyReLU(0.2, inplace=True) if not opt.use_gelu else nn.GELU()
