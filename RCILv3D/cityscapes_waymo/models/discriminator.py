...
import cityscapes_waymo.models.norms as norms
    ...
    def __init__(self, opt):
        ...
        self.dis = nn.ModuleList([
            nn.Sequential(
                ...
                nn.LeakyReLU(0.2, False) if not opt.use_gelu else nn.GELU(),
    ...
    def __init__(self, fin, fout, opt, up_or_down, first=False):
        ...
        if first:
            ...
            if self.up_or_down > 0:
                self.conv1 = nn.Sequential(nn.LeakyReLU(0.2, False) if not opt.use_gelu else nn.GELU(), nn.Upsample(scale_factor=2), norm_layer(nn.Conv2d(fin, fmiddle, 3, 1, 1)))
            else:
                self.conv1 = nn.Sequential(nn.LeakyReLU(0.2, False) if not opt.use_gelu else nn.GELU(), norm_layer(nn.Conv2d(fin, fmiddle, 3, 1, 1)))
        self.conv2 = nn.Sequential(nn.LeakyReLU(0.2, False) if not opt.use_gelu else nn.GELU(), norm_layer(nn.Conv2d(fmiddle, fout, 3, 1, 1)))
